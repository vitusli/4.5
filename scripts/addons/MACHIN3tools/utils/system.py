...
def get_autosave_external_folder():
    is_custom = False

    if (custom_folder := get_prefs().autosave_external_folder) and os.path.exists(custom_folder):
        folder = custom_folder
        is_custom = True

    else:
        folder = get_temp_dir(bpy.context)

    return folder, is_custom

...
class ToggleVIEW3DRegion(bpy.types.Operator):
    ...
    def initiate_asset_browser_area_settings(self, context, debug=False):
        ...
        if context.screen.name not in self.prefs:
            ...
            for region_type in ['ASSET_TOP', 'ASSET_BOTTOM']:
                if 'display_type' not in self.prefs[context.screen.name][region_type]:
                    self.prefs[context.screen.name][region_type]['display_type'] = 'THUMBNAIL'

                if bpy.app.version >= (4, 5, 0):
                    if 'list_display_size' not in self.prefs[context.screen.name][region_type]:
                        self.prefs[context.screen.name][region_type]['list_display_size'] = 32

                    if 'list_column_size' not in self.prefs[context.screen.name][region_type]:
                        self.prefs[context.screen.name][region_type]['list_column_size'] = 220
        ...
        for space in area.spaces:
            if space.type == 'FILE_BROWSER':
                if space.params:
                    ...
                    if bpy.app.version >= (4, 5, 0):
                        context.scene.M3['asset_browser_prefs'][screen_name][region_type]['list_display_size'] = space.params.list_display_size
                        context.scene.M3['asset_browser_prefs'][screen_name][region_type]['list_column_size'] = space.params.list_column_size
    ...
    def apply_asset_browser_area_settings(self, context, area, space, params, screen_name, region_type):
        if screen_name in self.prefs:
            ...
            if libref in get_registered_library_references(context):
                ...
                if bpy.app.version >= (4, 5, 0):
                    list_display_size = self.prefs[screen_name][region_type]['list_display_size']
                    list_column_size = self.prefs[screen_name][region_type]['list_column_size']

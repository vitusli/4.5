...
class SmartEdge(bpy.types.Operator):
    ...
    def description(cls, context, properties):
        if properties:
            if properties.sharp:
                return "Toggle Sharps"

            elif properties.offset:
                return "Offset Selected Edges"

            else:
                if tuple(context.scene.tool_settings.mesh_select_mode) == (True, False, False):
                    return "Knife Cut, Connect, Knife Project or Star Connect"

                elif tuple(context.scene.tool_settings.mesh_select_mode) == (False, True, False):
                    return "Loop Cut, Turn Edge, Select Region or Knife Project"

                elif tuple(context.scene.tool_settings.mesh_select_mode) == (False, False, True):
                    return "Select Bounds or Knife Project"
                else:
                    return "Smart Edge"

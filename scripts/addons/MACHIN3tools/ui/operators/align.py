...
from ... utils.math import average_locations, get_center_between_verts, create_rotation_difference_matrix, get_loc_matrix, create_coords_bbox, get_right_and_up_axes
from ... utils.selection import get_selected_vert_sequences, get_selection_islands
from ... utils.ui import popup_message
from ... items import axis_items, align_type_items, axis_index_mapping, align_direction_items, align_space_items, align_mode_items
...
class CenterEditMesh(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        self.center(context, axis_index_mapping[self.axis], self.direction, self.space)
        return {'FINISHED'}
    ...
    def center(self, context, axis, direction, space):
        ...
        if len(verts) == 1:
            ...
            _, centers, _ = create_coords_bbox([v.co for v in verts])
            origin = average_locations(centers)

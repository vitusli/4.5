...
def build_asset_browser_progress(ui: bpy.types.Panel,
    ...
    if show_cancel:
        op = row.operator(
            f"{PREFIX_OP}.cancel_asset_browser",
            text="",
            emboss=True,
            depress=0,
            icon="X"
...
class POLIIGON_PT_sidebar_left(bpy.types.Panel):
    bl_idname = f"{PREFIX_PT}_PT_sidebar_left"
    bl_label = "Poliigon"
...
class POLIIGON_PT_sidebar_right(bpy.types.Panel):
    bl_idname = f"{PREFIX_PT}_PT_sidebar_right"
    ...
    def draw(self, context):
        ...
        col = layout.column()
        row = col.row()
        row.operator(f"{PREFIX_OP}.asset_browser_reprocess",
        ...
        row = col.row(align=True)
        row.operator(
            f"{PREFIX_OP}.asset_browser_import",
            text=label_import,
            emboss=True,
        )
        row.operator(
            f"{PREFIX_OP}.asset_browser_quick_menu",
            text="",
            icon="TRIA_DOWN",
        )
...
if BUILD_OPTION_BOB:
    BOB_PT_sidebar_left = POLIIGON_PT_sidebar_left
    classes_bob = (
        # BOB_PT_sidebar_left,
    )

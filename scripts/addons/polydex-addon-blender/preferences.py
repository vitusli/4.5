...
class PoliigonPreferences(bpy.types.AddonPreferences):
    ...
    def _draw_additional_dirs_prefs(self) -> None:
        ...
        for directory in additional_dirs:
            ...
            op = row.operator(f"{PREFIX_OP}.poliigon_setting", text="", icon="TRASH")
        ...
        row = col.row(align=1)
        op = row.operator(
            f"{PREFIX_OP}.poliigon_directory",
    ...
    def _draw_display_prefs(self) -> None:
        ...
        col_right.operator(f"{PREFIX_OP}.refresh_data", icon="FILE_REFRESH")
    ...
    def _draw_updater_prefs(self) -> None:
        ...
        if cTB._updater.update_ready is not None:
            row.operator(f"{PREFIX_OP}.check_update",
                         text="", icon="FILE_REFRESH")

        subcol = row.column(align=True)
        if cTB._updater.is_checking:
            subcol.operator(f"{PREFIX_OP}.check_update",
                            text=_t("Checking..."))
...
if BUILD_OPTION_BOB:
    BOBPreferences = PoliigonPreferences
...
def register(addon_version: str) -> None:
    ...
    if BUILD_OPTION_P4B:
        ...
        bpy.utils.register_class(BOBPreferences)
...
def unregister() -> None:
    if BUILD_OPTION_P4B:
        ...
        bpy.utils.unregister_class(BOBPreferences)

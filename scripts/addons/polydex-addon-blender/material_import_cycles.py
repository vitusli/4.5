...
from .utils import (
    ...
    get_entity_props,
    get_window_manager_map_pref_props)
...
MAP_TYPE_TO_CYCLES_IMG_PARAMS = {
    ...
    MapType.UNDEF: ("sRGB", "STRAIGHT", "Linear"),
}
...
class CyclesNodesTopLevel():
    ...
    def get_undefined_tex_node(self) -> Optional[bpy.types.Node]:
        """Returns the 'Image Texture' node for undefined textures."""

        node_tex_undef = self.tex.get(MapType.UNDEF, None)
        return node_tex_undef
...
class CyclesMaterial():
    ...
    def configure_tex_node_image(
        ...
        if tex_map.size not in name_tex and tex_map.map_type != MapType.UNDEF:
            ...
            # For UNDEF maps (stemming from Polydex IMAGE or IMAGE_SET)
            # size is unknown.
    ...
    def create_tex_node(
        ...
        *,
        ...
        map_type_effective = tex_map.map_type.get_effective()
        if map_type_effective == MapType.UNDEF:
            # Never fold BOB's image nodes from "IMAGE" or "IMAGE_SET""
            hide = False
        elif map_type_effective in self.nodes.tex:
    ...
    def connect_color(self) -> None:
        ...
        if BUILD_OPTION_BOB and node_tex_color is None:
            node_tex_color = self.nodes.get_undefined_tex_node()
    ...
    def position_nodes(self) -> None:
        ...
        # Note: In P4B column 3 will always be populated for any
        #       reasonable material. Yet, for BOB it's not necessarily,
        #       but we want the gap nevertheless.
        #       Thus 'if column_populated' commented.
        # if column_populated:
        x_column -= X_OFFSET_COLUMN_WIDE
    ...
    def set_material_properties(self) -> None:
        ...
        props_mat = get_entity_props(mat)

        props_mat.asset_name = asset_name
        props_mat.asset_id = asset_id
        props_mat.asset_type = asset_type_name
        props_mat.size = size
        props_mat.mapping = projection
        props_mat.scale = scale
        props_mat.displacement = displacement
        props_mat.use_16bit = use_16bit
        props_mat.mode_disp = mode_disp

        if BUILD_OPTION_P4B:
            props_map_prefs = get_window_manager_map_pref_props(bpy.context)
            copy_simple_property_group(
                props_map_prefs, props_mat.map_prefs)

...
from .build import (
    ...
    NAME_PANEL_CATEGORY,
    PREFIX_ICON,
    PREFIX_FILENAME_BL_SETTINGS,
    PREFIX_FILENAME_SETTINGS)
            ...
            for window in wm.windows:
                for area in window.screen.areas:
                    ...
                    for region in area.regions:
                        ...
                        if region.active_panel_category != NAME_PANEL_CATEGORY:
                            continue
                        region.tag_redraw()
...
class c_Toolbox(PoliigonAddon):
    ...
    lock_thumbs = Lock()  # locks access to thumbs
    lock_client_start = Lock()
    lock_settings_file = Lock()
    ...
    def __init__(
        ...
        if environment.env_name is not None and environment.env_name == "test":
            settings_filename = f"{PREFIX_FILENAME_SETTINGS}settings_test.ini"
        else:
            settings_filename = f"{PREFIX_FILENAME_SETTINGS}settings.ini"
    ...
    def register(self, version: str) -> None:
        ...
        #     size_download=300,
        #     bmp_downloading=self.ui_icons[f"{PREFIX_ICON}ICON_get_preview"].icon_id,
        #     bmp_error=self.ui_icons[f"{PREFIX_ICON}ICON_no_preview"].icon_id,
        ...
        if BUILD_OPTION_P4B:
            ...
            self.bo_asset_counts: Dict[str, int] = {}

            self.bo_listener = BOSocketListener(self)
            self.bo_listener.start()

            # TODO(Patrick): This doesn't yet have the intended effect,
            # not actually loading other meshes like skp
            # self.polydex_supported_model = list(self._asset_index.addon_supported_model)
            # self._asset_index.addon_supported_model = [
            #     ModelType.FBX,
            #     ModelType.BLEND,
            #     # All other need to be listed so that we can display details about them
            #     ModelType.MAX,
            #     ModelType.C4D,
            #     ModelType.OBJ,
            #     ModelType.USD,
            ...
            print("toolbox.register: UNKNOWN BUILD OPTION")
    ...
    def _init_directories(self) -> None:
        ...
        if self._env.env_name is not None and self._env.env_name == "test":
            filename_settings = f"{PREFIX_FILENAME_BL_SETTINGS}Blender_Settings_test.ini"
        else:
            filename_settings = f"{PREFIX_FILENAME_BL_SETTINGS}Blender_Settings.ini"
    ...
    def _init_online_all_assets_page_1(self) -> None:
        ...
        if BUILD_OPTION_P4B:
            ...
            print("_init_online_all_assets_page_1: UNKNOWN BUILD OPTION")
        self.settings["area"] = tab
        ...
        props = get_window_manager_props()
    ...
    def callback_logout_done(self, job: ApiJob) -> None:
        ...
        if BUILD_OPTION_BOB:
            self.logger.critical(
                "callback_logout_done: BOB should never be here!")
            return
        ...
        op_base = get_op_base()
        op_base.poliigon_setting(mode="clear_email")
        op_base.poliigon_setting(mode="clear_pass")
    ...
    def signal_import_asset(self, asset_id: int, method: str = "") -> None:
        """Signals an asset import in the background if user opted in."""

        if not self._api._is_opted_in() or asset_id == 0:
            return
        if BUILD_OPTION_P4B:
            f_signal = self._api.signal_import_asset
            args = (asset_id,)
        else:
            f_signal = self.bo_listener.api_client.signal_import_asset
            args = (asset_id, method,)
        thread = Thread(
            target=f_signal,
            args=args,
        )
    ...
    def f_GetAssets(
        ...
        if BUILD_OPTION_P4B:
            ...
            idx_page_current = self.vPage[area]
            bo_get_assets(
                self.bo_listener,
                categories,
                search,
                idx_page=idx_page_current)
    ...
    def f_GetPageAssets(self, idx_page: int) -> Tuple[List[int], int]:
        ...
        if BUILD_OPTION_P4B:
            ...
            query_tuple = (area,
                           ...
                           idx_page,
                           num_per_page)
            asset_ids = self._asset_index.cached_queries.get(query_tuple, None)

            idx_first_asset = 0
            if asset_ids is not None:
                idx_last_asset = len(asset_ids)
                num_assets = self.bo_asset_counts[query_tuple]
                num_pages_bob = ceil(num_assets / num_per_page)
            else:
                idx_last_asset = 0
                num_pages_bob = 0
        else:
            print("f_GetPageAssets: UNKNOWN BUILD OPTION")
    ...
    def _get_asset_type_from_property(
        ...
        try:
            props_entity = get_entity_props(entity)
            asset_type_name = props_entity.asset_type
    ...
    def _find_asset_ids_in_scene(self) -> List[int]:
        ...
        for _entity_coll in [bpy.data.images,
            ...
            for _entity in _entity_coll:
                try:
                    props_entity = get_entity_props(_entity)
                    asset_id = props_entity.asset_id
    ...
    def f_GetSceneAssets(self):
        ...
        if BUILD_OPTION_P4B:
            ...
            query_base = (
                ...
                None,
                CATEGORY_ALL,
                search_imported if search_imported != "" else None,
                IDX_PAGE_ACCUMULATED,
                PAGE_SIZE_ACCUMULATED)
            query_key_imported = (
                KEY_TAB_IMPORTED,
            ...
            print("f_GetSceneAssets: UNKNOWN BUILD OPTION")
    ...
    def f_GetPreview(self,
        ...
        if path_thumb is not None and os.path.isfile(path_thumb):
            ...
            return self.ui_icons[f"{PREFIX_ICON}ICON_no_preview"].icon_id
    ...
    def update_files(self, path: str) -> bool:
        ...
        for f in files_to_update:
            ...
            # While renaming append a BOB prefix to all icons, so we do not
            # run into issues loading them into Blender previews.
            if BUILD_OPTION_BOB and f_split[1] == ".png":
                tgt_file = PREFIX_ICON + tgt_file
...
def f_tick_handler() -> float:
    ...
    addon_ctx = get_context(addon_version="0.0")

    next_call_s = 60  # Long to prevent frequent checks for updates.
    if not addon_ctx.vRunning:
        return next_call_s

    # Thread cleanup.
    for vT in list(addon_ctx.threads):
        if not vT.is_alive():
            addon_ctx.threads.remove(vT)

    # Updater callback.
    if addon_ctx.prefs and addon_ctx.prefs.auto_check_update:
        if addon_ctx._updater.has_time_elapsed(hours=24):
            addon_ctx._updater.async_check_for_update(
                addon_ctx.check_update_callback,
                create_notifications=True)

    return next_call_s


def f_do_on_main_thread() -> Optional[float]:
    ...
    if BUILD_OPTION_BOB:
        addon_ctx.logger.critical(
            "callback_logout_done: BOB should never be here!")
        return None

    if addon_ctx.msg_download_limit is not None:
        op_base = get_op_base()
        op_base.popup_download_limit(
            "INVOKE_DEFAULT", msg=addon_ctx.msg_download_limit)
        addon_ctx.msg_download_limit = None
...
def f_load_handler(*args) -> None:
    ...
    if addon_ctx.vRunning:
        addon_ctx.f_GetSceneAssets()
...
def init_context_p4b(addon_version: str) -> None:
    global cTB

    cTB = c_Toolbox(addon_version)


def init_context_bob(addon_version: str) -> None:
    global addon_bob

    addon_bob = c_Toolbox(addon_version)
    addon_bob._updater.base_url = "https://software.poliigon.com"


def get_context_p4b(addon_version: str) -> c_Toolbox:
    global cTB

    if cTB is None:
        init_context_p4b(addon_version)
    return cTB


def get_context_bob(addon_version: str) -> c_Toolbox:
    global addon_bob

    if addon_bob is None:
        init_context_bob(addon_version)
    return addon_bob


def get_context(addon_version: str) -> c_Toolbox:
    if BUILD_OPTION_P4B:
        return get_context_p4b(addon_version)
    elif BUILD_OPTION_BOB:
        return get_context_bob(addon_version)


def shutdown_asset_browser_client(addon_ctx) -> None:
    """Shuts down client Blender process"""

    addon_ctx.asset_browser_jobs_cancelled = True
    addon_ctx.asset_browser_quitting = True

    # Needed for some unit tests to not cause exception on shutdown
    # if ran in "single" mode
    if not addon_ctx.register_success:
        return

    if addon_ctx.proc_blender_client is not None:
        addon_ctx.proc_blender_client.terminate()


def shutdown_thumb_prefetch(addon_ctx) -> None:
...
def shutdown_addon(addon_ctx) -> None:
    """Shuts down all addon services.
    _Afterwards_ everything is ready for unregister.
    """

    addon_ctx.quitting = True
    shutdown_thumb_prefetch(addon_ctx)
    shutdown_asset_browser_client(addon_ctx)
    addon_ctx.api_rc.shutdown()

    if BUILD_OPTION_BOB:
        addon_ctx.bo_listener.shutdown()

    addon_ctx.vRunning = 0
...
def blender_quitting() -> None:
    ...
    if addon_ctx.vRunning == 1 and not addon_ctx.quitting:
        shutdown_addon(addon_ctx)


def register(addon_version: str) -> None:
    addon_ctx = get_context(addon_version)
    addon_ctx.register(addon_version)
...
def unregister(addon_ctx) -> None:
    global cTB
    global addon_bob

    shutdown_addon(addon_ctx)

    if addon_ctx.log_manager.file_handler is not None:
        addon_ctx.log_manager.file_handler.close()
    ...
    # for vT in addon_ctx.threads:
    #    vT.join()

    addon_ctx.ui_icons.clear()
    try:
        bpy.utils.previews.remove(addon_ctx.ui_icons)
    except KeyError:
        pass

    with addon_ctx.lock_thumbs:
        addon_ctx.thumbs.clear()

        try:
            bpy.utils.previews.remove(addon_ctx.thumbs)
        except KeyError:
            pass


def unregister_p4b() -> None:
    global cTB
...
def unregister_bob() -> None:
    global addon_bob

    if addon_bob is None:
        print("addon_bob is None during unregister")
        return

    unregister(addon_bob)
    addon_bob = None

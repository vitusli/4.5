...
def find_identical_material(asset_data: AssetData,
    ...
    for mat in bpy.data.materials:
        ...
        if not props_mat.asset_name.startswith(asset_name):
            continue
        if props_mat.asset_type != asset_type_imported:
            continue
        if props_mat.size != size:
            continue
        if props_mat.mapping != mapping:
            continue
        if props_mat.scale != scale:
            continue
        if props_mat.displacement != displacement:
            continue
        if props_mat.use_16bit != use_16bit:
            continue
        if props_mat.mode_disp != mode_disp:
            continue
        if props_mat.is_backplate != is_backplate:
            continue

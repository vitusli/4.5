...
from .utils import get_window_manager_map_pref_props
...
def add_map_pref_rows(cTB, pref, col: bpy.types.UILayout, context) -> None:
    ...
    map_pref_props = get_window_manager_map_pref_props(context)
...
class MapPrefProperties(bpy.types.PropertyGroup):
    ...
    def _get_file_format_options(
        ...
        if user is None:
            return []
        user_prefs = user.map_preferences
        if user_prefs is None:
            return []
...
if BUILD_OPTION_BOB:
    BOBMapPrefProperties = MapPrefProperties
...
def register(addon_version: str) -> None:
    ...
    if BUILD_OPTION_P4B:
        ...
        bpy.types.WindowManager.bob_map_prefs = bpy.props.PointerProperty(
            type=BOBMapPrefProperties)

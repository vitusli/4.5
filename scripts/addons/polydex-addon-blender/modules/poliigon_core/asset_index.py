...
class AssetIndex():
    ...
    def _populate_default_asset_info(self,
        ...
        rcs = asset_dict.get("render_custom_schema", {})
        if type(rcs) is list:
            rcs = {}
            # Just in case somebody else evaluates rcs from this dict...
            asset_dict["render_custom_schema"] = rcs
    ...
    def _analyze_single_file(self,
        ...
        if map_type is not None and size is not None:
            ...
            mesh = assets.ModelMesh(model_type=assets.ModelType.SKP,
                                    lod=lod,
                                    filename=filename,
                                    directory=path)
            meshes.append(mesh)
        elif suffix == ".obj":
            mesh = assets.ModelMesh(model_type=assets.ModelType.OBJ,
                                    lod=lod,
                                    filename=filename,
                                    directory=path)
            meshes.append(mesh)
        elif suffix == ".stl":
            mesh = assets.ModelMesh(model_type=assets.ModelType.STL,
                                    lod=lod,
                                    filename=filename,
                                    directory=path)
            meshes.append(mesh)
        elif suffix in [".usd", ".usdc", ".usda"]:
            mesh = assets.ModelMesh(model_type=assets.ModelType.USD,
                                    lod=lod,
                                    filename=filename,
                                    directory=path)
            meshes.append(mesh)
        elif suffix == [".glb", ".gltf"]:
            mesh = assets.ModelMesh(model_type=assets.ModelType.GLTF,
                                    lod=lod,
                                    filename=filename,
                                    directory=path)
            meshes.append(mesh)

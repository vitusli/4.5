...
class POLIIGON_OT_model(Operator):
    bl_idname = f"{PREFIX_OP}.poliigon_model"
    ...
    def execute(self, context):
        ...
        if self.exec_count == 0:
            cTB.signal_import_asset(asset_id=self.asset_id, method="object")
        self.exec_count += 1
        return {"FINISHED"}
    ...
    def set_poliigon_props_model(
        ...
        props_obj = get_entity_props(obj)
        props_obj.asset_id = self.asset_data.asset_id
        props_obj.asset_name = self.asset_data.asset_name
        props_obj.asset_type = self.asset_data.asset_type.name
        props_obj.size = self.size
        if lod is not None:
            props_obj.lod = lod
        props_obj.use_collection = self.do_use_collection
        props_obj.link_blend = self.do_link_blend

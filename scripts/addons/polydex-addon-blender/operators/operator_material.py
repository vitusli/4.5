...
class POLIIGON_OT_material(Operator):
    bl_idname = f"{PREFIX_OP}.poliigon_material"
    bl_label = _t(f"{NAME_PANEL} Material Import")
    ...
    def _get_dispopts(self, context):
        ...
        if is_cycles():
            ...
            # below is still only relevant in cycles.
            options.append(
                ("MICRO",
    ...
    def draw(self, context):
        ...
        is_enabled = True
        if BUILD_OPTION_BOB:
            is_image = self.asset_data.runtime.get_is_polydex_image()
            is_stack = self.asset_data.runtime.get_is_polydex_stack()
            if is_image and not is_stack:
                # No redo last options for single image
                return
            is_enabled = not is_image
    ...
    def signal_import(self) -> None:
        if self.exec_count == 0:
            cTB.signal_import_asset(asset_id=self.asset_id, method="material")
        self.exec_count += 1
    ...
    def execute(self, context):
        ...
        # TODO(Andreas): Can do_rename be removed, seems not in use at all?
        if BUILD_OPTION_P4B and self.do_rename:
        ...
        if self.ignore_map_prefs or BUILD_OPTION_BOB:
            map_prefs = None
        else:

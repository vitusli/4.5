...
class POLIIGON_OT_hdri(Operator):
    bl_idname = f"{PREFIX_OP}.poliigon_hdri"
    ...
    def _fill_light_size_drop_down(
        ...
        if BUILD_OPTION_BOB:
            items_size = []
            avail_maps:TextureMap = asset_type_data.light.maps["METALNESS"]
            for _map in avail_maps:
                # Tuple: (id, name, description, icon, enum value)
                size = _map.size
                ext = _map.file_format.replace(".", "").upper()
                items_size.append((size, f"{size} {ext}", f"{size} {ext}"))
            return items_size
    ...
    size_bg: EnumProperty(
        ...
        description=_t("Change size of background texture."),
        options={"HIDDEN"} if BUILD_OPTION_BOB else {})  # noqa F722
    ...
    def get_imported_hdri(asset_id: int) -> Optional[bpy.types.Image]:
        ...
        for _img in bpy.data.images:
            try:
                props_img = get_entity_props(_img)
                asset_id_img = props_img.asset_id
    ...
    def execute(self, context):
        ...
        if BUILD_OPTION_P4B:
            # Only P4B supports the background option for now.
            try:
        ...
        if not light_exists or not bg_exists:
            ...
            if len(files_tex_exr) == 0 and BUILD_OPTION_P4B:
                ...
                return {"CANCELLED"}
            file_tex_light = files_tex_exr[0] if BUILD_OPTION_P4B else None


            if BUILD_OPTION_BOB:
                # Identify here the light based on the import group
                # TODO: this is a bit more brittle than just getting list of
                # files  for a given import group from the source data.
                target_files = [_file for _file in files
                              if size_light.lower() in os.path.basename(_file).lower()]
                if not target_files and len(files) > 0:
                    # Fallback for when there are HDRIs with no size/group info
                    target_files = files
                if len(target_files) == 0:
                    ...
                    reporting.capture_message(
                        "failed_load_light_hdri", msg, "error")
                    msg = _t(
                        "Unable to locate image {0} with size {1}"
                    ).format(name_light, size_light)
                    self.report({"ERROR"}, msg)
                    return {"CANCELLED"}
                file_tex_light = target_files[0]
                size_bg = size_light
                file_tex_bg = file_tex_light
            elif cTB.settings["hdri_use_jpg_bg"] and filetype_bg == "JPG":
        ...
        if self.exec_count == 0:
            cTB.signal_import_asset(asset_id=self.asset_id, method="sky")
        self.exec_count += 1
    ...
    def set_poliigon_props_image(
        ...
        props_img = get_entity_props(img)
        props_img.asset_name = asset_data.asset_name
        props_img.asset_id = self.asset_id
        props_img.asset_type = asset_data.asset_type.name
        props_img.size = self.size
        props_img.size_bg = self.size_bg
        props_img.hdr_strength = self.hdr_strength
        props_img.rotation = self.rotation

    def set_poliigon_props_world(self, context, asset_data: AssetData) -> None:
        """Sets Poliigon property of world."""

        props_world = get_entity_props(context.scene.world)
        props_world.asset_name = asset_data.asset_name
        props_world.asset_id = self.asset_id
        props_world.asset_type = asset_data.asset_type.name
        props_world.size = self.size
        props_world.size_bg = self.size_bg
        props_world.hdr_strength = self.hdr_strength
        props_world.rotation = self.rotation

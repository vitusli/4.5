...
if BUILD_OPTION_BOB:
    from .barn_owl import (
        build_barn_owl_not_found,
        build_barn_owl_alpha_ended)
...
def build_ui(ui, context):
    ...
    props_wm = get_window_manager_props()
    ...
    if BUILD_OPTION_P4B:
        ...
        if cTB.bo_listener.bo_client.api_client.has_alpha_ended():
            build_barn_owl_alpha_ended(cTB)
            return
        if not cTB.bo_listener.bo_client.api_client.is_barnowl_detected():
            build_barn_owl_not_found(cTB)
            return
    else:
        print("build_ui: Unknown build option")
    ...
    if primary_lib_path in [None, ""] and BUILD_OPTION_P4B:
        build_library(cTB)
        return
...
def f_AssetInfo(cTB, asset_id: int):
    ...
    def asset_info_draw(self, context):
        ...
        op = row.operator(
            f"{PREFIX_OP}.poliigon_asset_options", text="", icon="FILE_FOLDER"
        )
...
class POLIIGON_PT_toolbox(Panel):
    bl_idname = f"{PREFIX_PT}_PT_toolbox"
    bl_label = NAME_PANEL
    bl_category = NAME_PANEL_CATEGORY
...
if BUILD_OPTION_BOB:
    BOB_PT_toolbox = POLIIGON_PT_toolbox
    BOB_MT_add_node_groups = POLIIGON_MT_add_node_groups
    append_poliigon_groups_node_add = bob_append_poliigon_groups_node_add

    classes = (
        BOB_PT_toolbox,
        BOB_MT_add_node_groups
    )
else:
    append_poliigon_groups_node_add = p4b_append_poliigon_groups_node_add

...
def get_window_manager_props():
    """Returns properties stored in window manager based upon either
    being P4B or BOB.
    """

    if BUILD_OPTION_P4B:
        return bpy.context.window_manager.poliigon_props
    elif BUILD_OPTION_BOB:
        return bpy.context.window_manager.bob_props


def get_entity_props(entity):
    """Returns properties stored in entities (objects, materials,...) based
    upon either being P4B or BOB.
    """

    if BUILD_OPTION_P4B:
        return entity.poliigon_props
    elif BUILD_OPTION_BOB:
        return entity.bob_props


def get_window_manager_map_pref_props(context):
    """Returns properties regarding map prefs stored in window manager based
    upon either being P4B or BOB.

    # TODO(Andreas): We probably will not have map prefs in BOB. Yet, for the
                     moment just having these existing leads to less changes
                     in importing code.
    """

    if BUILD_OPTION_P4B:
        return context.window_manager.polligon_map_prefs
    elif BUILD_OPTION_BOB:
        return context.window_manager.bob_map_prefs


def get_op_base():
    if BUILD_OPTION_P4B:
        return bpy.ops.poliigon
    elif BUILD_OPTION_BOB:
        return bpy.ops.bob

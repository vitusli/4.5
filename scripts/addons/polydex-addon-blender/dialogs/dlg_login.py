...
from ..utils import get_window_manager_props
...
def _draw_switch_email_login(col: bpy.types.UILayout,
    ...
    op_login_email = row_login_email.operator(f"{PREFIX_OP}.poliigon_user",
...
def _draw_browser_login(cTB: c_Toolbox, col: bpy.types.UILayout) -> None:
    if cTB.login_in_progress:
        ...
        op_login_website = col1.operator(f"{PREFIX_OP}.poliigon_user",
        ...
        op_login_cancel = col2.operator(f"{PREFIX_OP}.poliigon_user",
                                        text="",
                                        icon="X")
        ...
        op_login_website = row_button.operator(f"{PREFIX_OP}.poliigon_user",
...
def _draw_email_login(cTB: c_Toolbox, col: bpy.types.UILayout) -> None:
    ...
    row.prop(props_wm, "vEmail")

    col_x = row.column(align=True)
    op = col_x.operator(f"{PREFIX_OP}.poliigon_setting",
                        text="",
                        icon="X")
    ...
    if error_login and "@" not in props_wm.vEmail:
    ...
    if cTB.settings["show_pass"]:
        row.prop(props_wm, "vPassShow")
        vPass = props_wm.vPassShow

    else:
        row.prop(props_wm, "vPassHide")
        vPass = props_wm.vPassHide
    ...
    enable_login_button = len(props_wm.vEmail) > 0 and len(vPass) > 0

    row = col.row()
    row.scale_y = 1.25

    if cTB.login_in_progress:
        op_login = row.operator(f"{PREFIX_OP}.poliigon_setting",
                                text=_t("Logging In..."),
        ...
        op_login = row.operator(f"{PREFIX_OP}.poliigon_user",
                                text=_t("Login via email"))
        op_login.mode = "login"
    ...
    op_forgot = col.operator(f"{PREFIX_OP}.poliigon_link",
                             text=_t("Forgot Password?"),
                             emboss=False)
    op_forgot.mode = "forgot"
    op_forgot.tooltip = _t("Reset your Poliigon password")

    op_login_website = col.operator(f"{PREFIX_OP}.poliigon_user",
                                    text=_t("Login via Browser"),
                                    emboss=False)
...
def _draw_signup(cTB, layout: bpy.types.UILayout) -> None:
    ...
    op_signup = layout.operator(f"{PREFIX_OP}.poliigon_link",
                                text=_t("Sign Up"))
...
def _draw_legal(layout: bpy.types.UILayout) -> None:
    ...
    op_terms = col.operator(f"{PREFIX_OP}.poliigon_link",
                            text=_t("Terms & Conditions"),
                            emboss=False)
    op_terms.tooltip = _t("View the terms and conditions page")
    op_terms.mode = "terms"

    op_privacy = col.operator(f"{PREFIX_OP}.poliigon_link",

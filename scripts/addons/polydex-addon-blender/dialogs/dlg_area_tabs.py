...
def _draw_unlimited_icon(cTB, *, row: bpy.types.UILayout) -> None:
    ...
    op_icon = row.operator(
        f"{PREFIX_OP}.poliigon_setting", text="", emboss=True, icon_value=icon_value)
...
def _draw_asset_balance(cTB, *, row: bpy.types.UILayout) -> None:
    ...
    balance_icon = cTB.ui_icons[f"{PREFIX_ICON}ICON_asset_balance"].icon_id
    if cTB.is_paused_subscription() and credits <= 0:
        balance_icon = cTB.ui_icons[f"{PREFIX_ICON}ICON_subscription_paused"].icon_id
...
def _add_asset_tab(cTB,
    ...
    not_tab_user = not cTB.settings["show_user"]
    not_settings = not cTB.settings["show_settings"]
    not_user_or_settings = not_tab_user and not_settings
    ...
    op = col.operator(
        ...
        depress=is_tab_active and not_user_or_settings
    )
    op.mode = mode
    op.tooltip = tooltip

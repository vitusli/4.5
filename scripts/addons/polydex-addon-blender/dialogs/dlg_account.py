...
def _build_section_free_user(cTB) -> None:
    ...
    op = col.operator(f"{PREFIX_OP}.poliigon_link", text=_t("View Pricing"))
    op.mode = "subscribe"
...
def _build_section_paid_plan(cTB) -> None:
    ...
    op = col.operator(f"{PREFIX_OP}.poliigon_link", text=_t("View Details"))
    op.mode = "credits"
    # TODO(Andreas): Figma did not contain any tooltips...
...
def build_user(cTB) -> None:
    ...
    cTB.vBase.separator()
    op = cTB.vBase.operator(f"{PREFIX_OP}.poliigon_user", text=_t("Log Out"))
    op.mode = "logout"

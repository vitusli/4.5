    ...
    def draw(self, context):
        ...
        if asset_data.is_purchased or is_free or cTB.is_unlimited_user():
            ...
            if asset_data.runtime.get_is_polydex_image():
                ...
                op.do_as_planes = False

                label_img += _t(" as Planes")
        ...
        if not hide_detail_view and not BUILD_OPTION_BOB:
            ...
            if bpy.app.version >= (4, 2) and not is_osx:
                op_name = f"{PREFIX_OP}.detail_view_open"
                op_text = _t("View Asset Details")
            else:
                op_name = f"{PREFIX_OP}.view_thumbnail"
            ...
            # op.tooltip = _t("View larger thumbnail for {0}").format(asset_name)  # from BOB branch
            op.asset_id = asset_data.asset_id
            # op.thumbnail_index = 0  # from BOB branch
        ...
        if not BUILD_OPTION_BOB:
            ...
            row = layout.row()
            op = row.operator(
                f"{PREFIX_OP}.update_asset_browser",
    ...
    def draw_material_sizes(
        ...
        if imported or all_expected_maps_for_size:
            ...
            if not asset_data.runtime.get_is_polydex_image():
                ...
                if imported:
                    label = _t("Apply Material")
                    tip = _t("Apply Material\n{0}").format(asset_name)
                elif context.selected_objects:
                    label = _t("Import and Apply Material")
                    tip = _t("Apply Material\n{0}").format(asset_name)
                else:
                    label = _t("Import Material")
                    tip = _t("Import Material\n{0}").format(asset_name)

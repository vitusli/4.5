...
def p4b_append_poliigon_groups_node_add(self, context) -> None:
    """Appending to add node menu, for Poliigon node groups"""

    self.layout.menu("POLIIGON_MT_add_node_groups")


def bob_append_poliigon_groups_node_add(self, context) -> None:
    """Appending to add node menu, for Poliigon node groups"""

    self.layout.menu("BOB_MT_add_node_groups")


class POLIIGON_MT_add_node_groups(bpy.types.Menu):
    """Menu for the Poliigon Shader node groups"""

    bl_idname = f"{PREFIX_MT}_MT_add_node_groups"
    ...
    def draw(self, context):
        ...
        if bpy.app.version >= (2, 90):
            col.operator(f"{PREFIX_OP}.add_converter_node",
                         text=_t("Mosaic")

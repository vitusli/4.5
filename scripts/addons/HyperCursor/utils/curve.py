...
def verify_curve_data(data, type=''):
    ...
    elif type == 'is_entire_spline_selected':
        spline = data['active']
        selection = data['active_selection']
        return len(selection) == len(spline['points'])
    ...
    elif type == 'is_active_selection_continuous_with_cyclic_support':
        spline = data['active']
        selection = data['active_selection']

        if all(point['index'] == selection[idx]['index'] + 1 for idx, point in enumerate(selection[1:])):
            return True

        elif spline['cyclic']:

            if len(selection) == len(spline['points']):
                return False

            elif (spline['points'][0]['select'] and spline['points'][-1]['select']):
                sel1 = []
                sel2 = None
                ...
                for idx, point in enumerate(spline['points']):
                    if point['select']:
                        ...
                        sel2 = [p for p in spline['points'][idx:] if p['select']]
                        break

                return all(point['index'] == sel1[idx]['index'] + 1 for idx, point in enumerate(sel1[1:])) and all(point['index'] == sel2[idx]['index'] + 1 for idx, point in enumerate(sel2[1:]))

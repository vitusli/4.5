...
def transform_coords(coords, mx):
    coords_4d = np.ones((len(coords), 4), dtype=float)
    coords_4d[:, :-1] = coords

    transformed_coords = np.einsum('ij,aj->ai', mx, coords_4d)[:, :-1]
    return np.float32(transformed_coords)

def transform_batch(batch, mx):
    transformed_coords = transform_coords(batch[0], mx)

    if len(batch) == 2:
        return transformed_coords, batch[1]

    else:
        return transformed_coords, batch[1], batch[2]

def mirror_coords(coords, origin, direction):
    mirror_point = np.array(origin)
    mirror_normal = np.array(direction)

    mirror_normal = mirror_normal / np.linalg.norm(mirror_normal)

    difference_vectors = coords - mirror_point

    dot_products = np.dot(difference_vectors, mirror_normal)

    projections = np.outer(dot_products, mirror_normal)

    mirrored_coords = coords - 2 * projections

    return np.float32(mirrored_coords)

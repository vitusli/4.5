...
def cast_bvh_ray_from_mouse(mousepos, candidates=None, bmeshes={}, bvhs={}, debug=False):
    ...
    clamp = 1000 if bpy.context.region_data.view_perspective == 'ORTHO' else None

    origin_3d = region_2d_to_origin_3d(region, region_data, mousepos, clamp=clamp)
...
def get_hittable_scene_cast_objects(context, exclude=[], exclude_wire=False, debug=False):
    scene = context.scene

    hittable_objects = set(obj for obj in context.visible_objects if obj.display_type != 'BOUNDS')
    ...
    return hittable_objects, instance_collection_map

def cast_scene_ray_from_mouse(mousepos, depsgraph, exclude=[], exclude_wire=False, unhide=[], cache={}, debug=False):

    region = bpy.context.region
    region_data = bpy.context.region_data
    clamp = 1000 if bpy.context.region_data.view_perspective == 'ORTHO' else None

    origin = region_2d_to_origin_3d(region, region_data, mousepos, clamp=clamp)
    direction = region_2d_to_vector_3d(region, region_data, mousepos)

    return cast_scene_ray(origin, direction, depsgraph, exclude=exclude, exclude_wire=exclude_wire, unhide=unhide, cache=cache, debug=debug)

def cast_scene_ray(origin, direction, depsgraph, exclude=[], exclude_wire=False, unhide=[], cache={}, debug=False):
    debug = False
    ...
    if cache:
        ...
        hittable_objects, instance_collection_map = get_hittable_scene_cast_objects(context, exclude=exclude, exclude_wire=exclude_wire, debug=debug)

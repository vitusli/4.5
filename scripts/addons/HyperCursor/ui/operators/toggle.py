...
from ... utils.modifier import source_poll
...
class ToggleGeometryGizmos(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if source_poll(context, active):
            draw_fading_label(context, text=f"{active.name} is sourcing another object's mesh. Its own Geometry Geometry Gizmos are therefore hidden!", color=(1, 0.3, 0), alpha=1, move_y=40, time=4)
            return {'CANCELLED'}
...
class ToggleEditMode(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if active.HC.ishyperbevel:
            draw_fading_label(context, text="HyperBevel objects can't be switched to SCALE Geometry Gizmo mode", color=red, alpha=1, move_y=20, time=2)
            return {'CANCELLED'}

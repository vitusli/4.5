    ...
    def draw(self, context):
        ...
        if area in screen_areas:
            ...
            with context.temp_override(area=area):
                if context.active_file:
                    icon_id = context.active_file.preview_icon_id
        ...
        if pretty_path:
            ...
            if icon_id is not None:
                column.separator(factor=8)

                row = column.row()
                row.alignment = 'LEFT'
                row.template_icon(icon_value=icon_id, scale=4)

            column.label(text=pretty_path)
...
class PieEditFace(Menu):
    ...
    def draw(self, context):
        ...
        op = col.operator("machin3.extrude_face", text="Extrude", icon='BLANK1')
        op.index = index
        op.is_push_invocation = False
        ...
        if punchit and getattr(bpy.types, 'MACHIN3_OT_punchit', False):
            col.separator()

            op = col.operator("machin3.punchit", text="Punch It", icon='BLANK1')
            op.objmode = True
            op.faceindex = index
        ...
        if machin3tools and getattr(bpy.types, 'MACHIN3_OT_transform_edge_constrained', False):
            ...
            op = col.operator("machin3.transform_edge_constrained", text="Rotate Edge Constrained", icon='BLANK1')
            op.transform_mode = 'ROTATE'
            op.objmode = True
            op.edgeindex = -1
            op.faceindex = index

...
from .. utils.ui import finish_modal_handlers, force_geo_gizmo_update, get_mouse_pos, init_modal_handlers, init_status, finish_status, force_ui_update, draw_status_item
...
def draw_scale_mesh_status(op):
    def draw(self, context):
        ...
        if op.uniform_scale:
            ...
            if op.direction in ['XMIN', 'XMAX']:
                axis = 'YZ'

            elif op.direction in ['YMIN', 'YMAX']:
                axis = 'XZ'
        ...
        if not op.cursor_space_rotation:
            draw_status_item(row, key='ALT', text="Origin", prop='Cursor' if op.cursor_space_location else 'Object')
...
class ScaleMesh(bpy.types.Operator):
    ...
    def draw_HUD(self, context):
        if context.area == self.area:
            ...
            dims += draw_label(context, title=f"{direction} ", coords=Vector((self.HUD_x + dims.x, self.HUD_y)), center=False, color=white, alpha=1)
            ...
            for axis, color in axes_colors:
                dims += draw_label(context, title=f"{axis}", coords=Vector((self.HUD_x + dims.x, self.HUD_y)), center=False, color=color, alpha=1)

            self.offset += 18

            across = f"{'Cursor' if self.cursor_space_rotation or self.cursor_space_location else 'Object Origin'}"
            dims = draw_label(context, title="across ", coords=Vector((self.HUD_x, self.HUD_y)), offset=self.offset, center=False, color=yellow, alpha=1)
            draw_label(context, title=f"{across}", coords=Vector((self.HUD_x + dims.x, self.HUD_y)), offset=self.offset, center=False, color=white, alpha=1)

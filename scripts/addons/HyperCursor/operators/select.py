...
from .. utils.ui import Mouse, force_geo_gizmo_update
...
class SelectEdge(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        edges = get_edges(edge, loop=self.loop, loop_min_angle=180 - self.loop_min_angle, loop_prefer_center_of_three=self.loop_prefer_center_of_three, ring=self.ring)
...
class SelectFace(bpy.types.Operator, Mouse):
    ...
    def get_closest_edge(self, context, mx, face, debug=False):
        ...
        view_origin, view_dir = get_view_origin_and_dir(context, mousepos)
        print("yo")

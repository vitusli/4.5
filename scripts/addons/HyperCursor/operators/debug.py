...
from .. utils.draw import draw_line, draw_lines, draw_point, draw_vector, draw_vectors, draw_batch, draw_circle
from .. utils.object import get_batch_from_mesh

from .. colors import yellow, green, red, normal, blue, white

class HyperCursorDebug(bpy.types.Operator):
    bl_idname = "machin3.hypercursor_debug"
    bl_label = "MACHIN3: HyperCursor Debug"
    bl_description = "Debug/Tesst"
...
class DrawTests(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if active:
            ...
            batch = get_batch_from_mesh(active.data, mx=active.matrix_world, offset=0.01)
            draw_batch(batch, color=yellow, width=1, alpha=0.5, xray=True, modal=False)

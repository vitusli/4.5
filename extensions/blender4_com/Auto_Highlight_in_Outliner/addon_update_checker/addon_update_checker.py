...
def get_current_version():
    ...
    return (
        str(sys.modules[addon].bl_info["version"])
        .replace("(", "")
        .replace(")", "")
        .replace(", ", "")
    )
...
def get_available_version():
    ...
    try:
        ...
        return str(results["files"]["Version Info"]["content"]).split("\n")[0], (
            ...
            if len(str(results["files"]["Version Info"]["content"]).split("\n")) > 1
            else ""
        )
        ...
        if version:
            ...
            version_tuple = tuple(
                [
                    ...
                    for a in version.replace(" ", "").split(".")
                ]
            )
...
def check_for_updates(force=False):
        ...
        if os.path.exists(os.path.join(os.path.dirname(__file__), "last_check.txt")):
            try:
                last_time = read_time_from_file(
                    os.path.join(os.path.dirname(__file__), "last_check.txt")
                )
                current_time_dt = datetime.fromisoformat(current_time_iso)
                time_from_file_dt = datetime.fromisoformat(last_time)
                if (
                    abs((current_time_dt - time_from_file_dt).days)
                    < preferences().check_every_days
                ):
    ...
    (
        preferences().update_available,
        preferences().online_version,
        preferences().update_message,
    ) = is_update_available()


def draw_update_button(layout):
    layout.operator("wm.url_open", text="Open Downloads Page", icon="EXPORT").url = (
        preferences().download_page_url
    )
...
def draw_update_section_for_prefs(layout, context=None):
    try:
        ...
        if preferences().update_available:
            box.label(
                text=f"Update is available!(v{preferences().online_version})",
                icon_value=pcoll["updatered"].icon_id,
            )
            ...
            if update_message:
                ...
                for line in preferences().update_message.split("\n"):
                    lines = textwrap.wrap(
                        line,
                        context.region.width / 10 if context else 100,
            ...
            box.label(
                text=(
                    f"Addon is Up to Date! (Last Checked at {preferences().last_check_time})"
                    if preferences().last_check_time
                    else f"Addon is Up to Date!"
                ),
                icon_value=pcoll["check"].icon_id,
            )
    except Exception as e:
        print(e)
...
class AddonUpdateChecker:
    ...
    download_page_url: bpy.props.StringProperty(
        default=f"https://blendermarket.com/account/orders_search?orders_search%5Bq%5D={get_addon_name()}",
    ...
    check_for_updates: bpy.props.BoolProperty(
        default=False, name="Check for Updates", update=check_for_updates_toggled
    )
    check_on_boot: bpy.props.BoolProperty(
        default=False, name="Check for Updates on Boot!"
    )
    last_check_time: bpy.props.StringProperty(default="")
    check_every_days: bpy.props.IntProperty(
        default=7,
        name="Check every X days",
        description="Check for Updates every X days",
    )

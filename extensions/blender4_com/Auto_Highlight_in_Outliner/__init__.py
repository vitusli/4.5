...
def savePreferences():
    ...
    with open(config_path, mode="w+", newline="\n", encoding="utf-8") as file:
        for p in preferences().__annotations__.keys():
            ...
            file.write(
                f"{p}=>{type(getattr(preferences(),p,'str')).__name__}==={getattr(preferences(),p)}\n"
            )
...
class AHPrefs(bpy.types.AddonPreferences, AddonUpdateChecker):
    ...
    expand_all_selected: bpy.props.BoolProperty(
        default=True,
        name="Keep all selected objects expanded when using 'Collapse other collections'",
        description="Enabling this option will expand all selected objects in the outliner. Disabling it will only expand the active object.",
    )
    expand_bones: bpy.props.BoolProperty(
        default=False, name="Expand to show selected bones in Pose mode"
    )
    ex: bpy.props.BoolProperty(name="Experimental", default=False)
    show_actual_object_for_parented_objects: bpy.props.BoolProperty(
        default=False,
        name="Show actual object for parented objects",
        description="When disabled, the addon will expand to show the grayed out version of children objects (the ones shown under the parent object) instead of the actual object, which may be located in another collection. Enabling this option will display the actual object from the other collection instead.",
    )
    default_state: bpy.props.BoolProperty(
        default=True,
        name="Enable by default in all new scenes",
        description="This option allows you to set the starting state for new scenes. When enabled, auto highlight will be turned on by default for every new Blender scene you open.",
    )
    collapse_fix: bpy.props.BoolProperty(
        default=False,
        name="Enable if performance is slow when using 'Collapse other collections'",
    )
    max_objects_to_expand: bpy.props.IntProperty(
        default=10, min=1, max=999999, name="Objects limit"
    )

    def draw(self, context):
        draw_update_section_for_prefs(self.layout, context)
        lines = textwrap.wrap(
            "Ensure that both of these keymaps are set to correspond with the select button you utilize in the viewport and the outliner!",
            context.region.width if context else 100,
        ...
        self.layout.label(
            text="(lower this if you are facing crashes when selecting a lot of objects)"
        )
        ...
        self.layout.prop(
            ...
            "ex",
            toggle=True,
            icon="DOWNARROW_HLT" if self.ex else "RIGHTARROW_THIN",
            emboss=False,
        )
        if self.ex:
...
def show_active_again(active, override):
    if bpy.context.mode == "OBJECT":
        if True or not (
            active
            and active.parent
            and set(active.parent.users_collection) == set(active.users_collection)
        ):
...
def show_active(active_bone, active, override, area, filter_children, filter_content):
    ...
    if preferences().expand_all_selected:
        ...
        if bpy.context.mode == "OBJECT":
            for a in bpy.context.selected_objects:
                ...
                if a != active and not (
                    a.parent
                    and set(a.parent.users_collection) == set(a.users_collection)
                ):
    ...
    if bpy.context.mode == "OBJECT":
        if active.parent and set(active.parent.users_collection) != set(
            active.users_collection
        ):
    ...
    if not expanded_active:
        bpy.app.timers.register(
            functools.partial(show_active_again, active, override), first_interval=0.01
        )
    return None
...
def highlight_in_outliner():
    st = time.time()
    if (
        bpy.context.scene.auto_highlight_in_outliner
        and not bpy.context.scene.auto_highlight_temp_disable
    ):
                ...
                for area in win.screen.areas:
                    if "OUTLINER" in area.type:
                        if (
                            ...
                            in ["VIEW_LAYER", "SCENES", "LIBRARIES"]
                        ):
                            for region in area.regions:
                                if "WINDOW" in region.type:
                                    ...
                                    if (
                                        bpy.context.scene.collapse_other_collections
                                        and bpy.context.selected_objects[:]
                                    ):
                                        if (
                                            True
                                            or not preferences().show_actual_object_for_parented_objects
                                        ):
                                    ...
                                    filter_children = (
                                        area.spaces.active.use_filter_children
                                    )
                                    filter_content = (
                                        area.spaces.active.use_filter_object_content
                                    )
                                    if (
                                        has_parent
                                        and preferences().show_actual_object_for_parented_objects
                                    ):
                                        count = 1
                                        if bpy.context.mode == "OBJECT":
                                            for a in bpy.context.selected_objects:
                                                if (
                                                    a != active
                                                    and a.parent
                                                    and set(a.parent.users_collection)
                                                    == set(a.users_collection)
                                                ):
                                                    count += 1
                                                    ...
                                                    if (
                                                        count
                                                        > preferences().max_objects_to_expand
                                                    ):
                                                        break
                                        if bpy.context.mode == "OBJECT":
                                            if (
                                                active
                                                and active.parent
                                                and set(active.parent.users_collection)
                                                == set(active.users_collection)
                                            ):
                                                if (
                                                    bpy.context.view_layer.objects.active
                                                    != active
                                                        ...
                                                        if a != active_bone and active:
                                                            ...
                                                            with bpy.context.temp_override(
                                                                **override
                                                                ...
                                                                if (
                                                                    preferences().show_actual_object_for_parented_objects
                                                                ):
                                                                    area.spaces.active.use_filter_children = (
                                                                        False
                                                                    )
                                        ...
                                        if preferences().expand_bones:
                                            if (
                                                ...
                                                active_bone.id_data.data.bones.active = (
                                                    active_bone.bone
                                                )
                                                with bpy.context.temp_override(
                                                    **override
...
def register():
    ...
    if kc:
        kmi = km.keymap_items.new(
            "ah.clickbypassview", type=f"LEFTMOUSE", value="PRESS", any=True
        )
        addon_keymaps.append((km, kmi))
    km = kc.keymaps.new(name="Outliner", space_type="OUTLINER")
    if kc:
        kmi = km.keymap_items.new(
            "ah.clickbypass", type="LEFTMOUSE", value="PRESS", any=True
        )
        addon_keymaps.append((km, kmi))
    bpy.types.OUTLINER_PT_filter.append(drawIntoOutliner)
    bpy.types.Scene.auto_highlight_in_outliner = bpy.props.BoolProperty(
        default=True, name="Auto Highlight", update=enabled
    )
    bpy.types.Scene.collapse_other_collections = bpy.props.BoolProperty(
        default=True, name="Collapse Other Collections"
    )
    bpy.types.Scene.auto_highlight_temp_disable = bpy.props.BoolProperty(
        default=False, name="Auto Highlight Temp Disable"
    )
    bpy.types.Scene.ah_state_set = bpy.props.BoolProperty(
        default=False, name="Auto Highlight State Set"
    )

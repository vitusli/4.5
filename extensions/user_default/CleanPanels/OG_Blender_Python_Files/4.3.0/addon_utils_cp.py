...
def _initialize_once():
    ...
    import os
    atl_file=ATL_FILE_PATH
    addons_to_load_on_boot=['bl_pkg','CleanPanels','io_anim_bvh', 'io_curve_svg', 'io_mesh_uv_layout', 'io_scene_fbx', 'io_scene_gltf2','cycles', 'ui_translate','node_wrangler','viewport_vr_preview','hydra_strom','rigify','pose_library','copy_global_transform']
    ...
    for addon in _preferences.addons:
        module=addon.module if not addon.module.startswith("bl_ext.") else addon.module.split('.')[2]
        if module in addons_to_load_on_boot:
            enable(
                addon.module,
...
def reset_all(*, reload_scripts=False,cp=False,addons_to_load=[]):
    """
    ...
    for path, pkg_id in _paths_with_extension_repos():
        ...
        for mod_name, _mod_path in _bpy.path.module_names(path, package=pkg_id):
            addon_name=mod_name if not mod_name.startswith("bl_ext.") else mod_name.split('.')[2]
            if cp or addon_name in addons_to_load_on_boot or os.path.dirname(os.path.dirname(os.path.dirname(sys.executable))) in _mod_path:
                if addons_to_load:
                    if addon_name not in addons_to_load:
                        continue

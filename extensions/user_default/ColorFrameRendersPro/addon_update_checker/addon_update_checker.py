...
# package_name=package_name.replace("bl_ext.","")
# prefix_pattern = r".*?\.(.+)"  # Match everything after the first dot
# match = re.match(prefix_pattern, package_name)
# if match:
#     package_name = match.group(1)
#     print(package_name)
# else:
#     print("Prefix pattern not found")
def get_addon_name():
    if "bl_ext" in __package__:
...
def get_current_version():
    ...
    mod_path=sys.modules[addon].__path__[0]
    filepath_toml = os.path.join(mod_path, 'blender_manifest.toml')
    if bpy.app.version>=(4,2,0) and os.path.isfile(filepath_toml):
        version='0.0.0'
        try:

...
@polib.log_helpers_bpy.logged_operator
class SceneSelectOutputFolder(
    output_path_format.PreviewSelectOutputFolderMixin,
    output_path_format.SelectOutputFolderMixin,
    bpy.types.Operator,
):
    ...
    def execute(self, context: bpy.types.Context):
        ...
        if (
            self.output_folder_attribute_name
            == output_path_format.OutputFormatProperty.OUTPUT_FOLDER.value
        ):
            return {'FINISHED'}
        elif (
            self.output_folder_attribute_name
            == output_path_format.OutputFormatProperty.PRESET_OUTPUT_FOLDER.value
        ):
            return bpy.ops.renderset.render_all_renderset_contexts(
                'INVOKE_DEFAULT', action=self.dialog_type, preview=True
            )
...
class ACTION_UL_ScenePostRenderActionList(
...
MODULE_CLASSES.append(ACTION_UL_ScenePostRenderActionList)


def on_update_collection_toggles(_, context: bpy.types.Context) -> None:
    if (
        renderset_context_utils.SKIP_RENDER_CONTEXT_SYNC
        or renderset_context_utils.SKIP_RENDER_CONTEXT_APPLY
    ):
        return

    active_rset_context = renderset_context_utils.get_active_renderset_context(context)
    if active_rset_context is None:
        return
    # Sync everything to the active context first
    active_rset_context.sync_collection_toggles(context)

    # Update all other contexts
    for rset_context in renderset_context_utils.get_all_renderset_contexts(context):
        if rset_context == active_rset_context:
            continue
        rset_context.apply_collection_toggles(context)
        rset_context.sync_collection_toggles(context)

    # Apply the active context back
    active_rset_context.apply_collection_toggles(context)


def on_update_view_layers_toggles(_, context: bpy.types.Context) -> None:
    if (
        renderset_context_utils.SKIP_RENDER_CONTEXT_SYNC
        or renderset_context_utils.SKIP_RENDER_CONTEXT_APPLY
    ):
    ...
    if active_rset_context is None:
        return
    # Sync everything to the active context first
    active_rset_context.sync_view_layers_toggles(context)

    # Update all other contexts
    for rset_context in renderset_context_utils.get_all_renderset_contexts(context):
        if rset_context == active_rset_context:
            continue
        rset_context.apply_view_layers_toggles(context)
        rset_context.sync_view_layers_toggles(context)

    # Apply the active context back
    active_rset_context.apply_view_layers_toggles(context)


def on_update_object_toggles(_, context: bpy.types.Context) -> None:
    if (
        renderset_context_utils.SKIP_RENDER_CONTEXT_SYNC
        or renderset_context_utils.SKIP_RENDER_CONTEXT_APPLY
    ):
    ...
    if active_rset_context is None:
        return
    # Sync everything to the active context first
    active_rset_context.sync_object_toggles(context)

    # Update all other contexts
    for rset_context in renderset_context_utils.get_all_renderset_contexts(context):
        if rset_context == active_rset_context:
            continue
        rset_context.apply_object_toggles(context)
        rset_context.sync_object_toggles(context)

    # Apply the active context back
    active_rset_context.apply_object_toggles(context)
...
class CollectionTogglesSettings(bpy.types.PropertyGroup):
    hide_select: bpy.props.BoolProperty(
        ...
        default=False,
        update=on_update_collection_toggles,
    )
...
class LayerCollectionTogglesSettings(bpy.types.PropertyGroup):
    exclude: bpy.props.BoolProperty(
        ...
        default=True,
        update=on_update_view_layers_toggles,
    )
    hide_viewport: bpy.props.BoolProperty(
...
class ObjectTogglesSettings(bpy.types.PropertyGroup):
    hide_select: bpy.props.BoolProperty(
        name="Remember Disable Selection",
        default=False,
        update=on_update_object_toggles,
    )
    hide_viewport: bpy.props.BoolProperty(
        name="Remember Disable in Viewports",
        default=False,

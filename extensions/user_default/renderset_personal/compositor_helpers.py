...
OUTPUT_FILE_NODE_LABEL = "Renderset Output (Layer Split)"
...
class RenderPassType(enum.Enum):
    ...
    # For user defined multilayer EXR output nodes
    MULTILAYER_EXR = "MultilayerEXR"
...
def ensure_file_output_node_exists(nodes: bpy.types.Nodes) -> bpy.types.CompositorNodeOutputFile:
    ...
    if node is None:
        ...
        node.label = OUTPUT_FILE_NODE_LABEL
    return node
...
def ensure_render_layers_file_out(scene: bpy.types.Scene, output_folder_path: str) -> None:
    ...
    for render_pass in get_scene_render_passes(scene, render_layers_node):
        ...
        if not polib.node_utils_bpy.is_node_socket_connected_to(
            links, render_layers_node, render_pass_socket_name, [file_output_node], None, True
        ):
...
def set_all_output_nodes_output_folder(
    scene: bpy.types.Scene,
    output_folder_path: str,
    multilayer_exr_prefix: typing.Optional[str] = None,
    include_renderset_output_node: bool = False,
) -> typing.Dict[str, str]:
    """Sets the output folder of all file output nodes in the compositor node tree.

    Excludes the renderset output node by default. Can be included by setting include_renderset_output_node to True.
    Returns a dict mapping the names of the file output nodes to their original output folder paths.
    """
    ret = {}
    ...
    for node in file_output_nodes:
        if node.name == OUTPUT_FILE_NODE_NAME and not include_renderset_output_node:
            continue

        assert node.name not in ret
        ret[node.name] = node.base_path
        # We need to clean the node name from characters that could break
        clean_node_name = polib.utils_bpy.filter_invalid_characters_from_path(
            node.name.replace("/", "_").replace("\\", "_")
        )
        node.base_path = os.path.join(output_folder_path, clean_node_name)
        # Output nodes with multilayer EXR name their files with the current frame
        # We can add a prefix to force the output to follow our naming convention
        if node.format.file_format == 'OPEN_EXR_MULTILAYER' and multilayer_exr_prefix is not None:
            node.base_path = os.path.join(node.base_path, multilayer_exr_prefix)
    return ret


def set_output_nodes_folder_paths_from_dict(
    scene: bpy.types.Scene, node_name_to_path_dict: typing.Dict[str, str]
) -> None:
    if not scene.use_nodes:
        logger.error("Scene doesn't use nodes!")
        return
    nodes = scene.node_tree.nodes
    for name, path in node_name_to_path_dict.items():
        node = nodes.get(name, None)
        if node is None:
            logger.error(f"Node {name} not found!")
            continue
        assert node.bl_idname == OUTPUT_FILE_NODE
        node.base_path = path

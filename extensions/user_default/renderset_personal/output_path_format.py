...
class OutputFormatProperty(enum.Enum):
    ...
    PRESET_OUTPUT_FOLDER = "preset_folder_path"
...
DEFAULT_PRESET_FOLDER_PATH = os.path.join(
    "{blend_parent_folder}",
    "previews",
    "{context_name}",
)
...
class OutputFormatProperties(bpy.types.PropertyGroup):
    ...
    def reset_preset_format(self, context: bpy.types.Context) -> None:
        self.preset_folder_path = DEFAULT_PRESET_FOLDER_PATH
    ...
    def ensure_not_empty_output_folder(self, context: bpy.types.Context) -> None:
        ...
        if self.preset_folder_path == "":
            self.reset_preset_format(context)
    ...
    preset_folder_path: bpy.props.StringProperty(
        name="Autogenerated Preset Output Folder Format",
        default=DEFAULT_PRESET_FOLDER_PATH,
        update=ensure_not_empty_output_folder,
        description=FORMAT_PROPERTY_DESCRIPTION,
        # We need custom folder picker in the lower versions
        # because the default crashes operator dialogs
...
class SelectOutputFolderMixin(polib.ui_bpy.SelectFolderPathMixin):
    output_folder_attribute_name: bpy.props.EnumProperty(
        name="Output Folder Attribute Name",
        items=[
            (
            ...
            (
                OutputFormatProperty.PRESET_OUTPUT_FOLDER.value,
                OutputFormatProperty.PRESET_OUTPUT_FOLDER.value,
                OutputFormatProperty.PRESET_OUTPUT_FOLDER.value,
            ),
        ],
    ...
    def apply_selected_folder_path(
        ...
        if folder_path is not None:
            setattr(output_format, self.output_folder_attribute_name, folder_path)
        if dialog_func is not None:
            return dialog_func('INVOKE_DEFAULT')
        return {'FINISHED'}


class PreviewSelectOutputFolderMixin:
    dialog_type: bpy.props.EnumProperty(
        items=[
            (utils.RenderAction.ALL, "None", "Render All Previews"),
            (utils.RenderAction.CURRENT, "None", "Render Current Preview"),
        ],
        default="all",
    )
...
def create_replacement_mapping(
    ...
    ret["frame_current"] = (
        f"{polib.utils_bpy.format_signed_int_padding(bpy.context.scene.frame_current)}"
        if frame_current is None
        else polib.utils_bpy.format_signed_int_padding(frame_current)
    )
    ret["frame_start"] = (
        polib.utils_bpy.format_signed_int_padding(bpy.context.scene.frame_start)
        if frame_start is None
        else polib.utils_bpy.format_signed_int_padding(frame_start)
    )
    ret["frame_end"] = (
        polib.utils_bpy.format_signed_int_padding(bpy.context.scene.frame_end)
        if frame_end is None
        else polib.utils_bpy.format_signed_int_padding(frame_end)
    )
    ret["frame_step"] = (
        polib.utils_bpy.format_signed_int_padding(bpy.context.scene.frame_step)
        if frame_step is None
        else polib.utils_bpy.format_signed_int_padding(frame_step)
    )
...
def generate_folder_path(
    ...
    ret = polib.utils_bpy.filter_invalid_characters_from_path(ret)
    # 5) resolve the ../ and ./
...
class OutputFormatAddVariableMixin:
    ...
    target: bpy.props.EnumProperty(
        ...
        items=(
            ...
            (
                OutputFormatProperty.PRESET_OUTPUT_FOLDER.value,
                "Autogenerated Preset Output Folder Path",
                "Add into preview output folder path in preview options popup",
            ),
            (
    ...
    def add_variable(self, output_props: OutputFormatProperties) -> None:
        ...
        elif self.target == OutputFormatProperty.PRESET_OUTPUT_FOLDER.value:
            if not output_props.preset_folder_path.endswith(("/", "\\")):
                token_to_append = os.path.sep + token_to_append
            output_props.preset_folder_path += token_to_append
...
def draw_output_folder_ui(
    ...
    add_variable_operator: OutputFormatAddVariableMixin,
    renderset_context: typing.Union["renderset_context.RendersetContext", MockRendersetContext],
    output_folder_attribute: OutputFormatProperty = OutputFormatProperty.OUTPUT_FOLDER,
    ...
    row.alignment = 'RIGHT'
    if output_folder_attribute == OutputFormatProperty.OUTPUT_FOLDER:
        row.prop_menu_enum(output_props, "folder_path_preset", text="Choose Preset")
    draw_add_variable_ui(row, add_variable_operator, output_folder_attribute)

    folder_row = main_col.row(align=True)
    folder_row.prop(output_props, output_folder_attribute.value, text="")
    if select_folder_operator is not None:
        select_folder_operator.draw_button(folder_row, icon='FILE_FOLDER', text="").filepath = (
            os.path.join(getattr(output_props, output_folder_attribute.value), os.path.sep)
        )
    valid, err_msg = is_output_path_valid(getattr(output_props, output_folder_attribute.value))

    # we don't want to display Existing vs. nonexisting path for previews
    if output_folder_attribute == OutputFormatProperty.PRESET_OUTPUT_FOLDER:
        return
    ...
    if bpy.data.filepath == "":  # blend file is not saved
        ...
        path_preview = generate_folder_path(
            getattr(output_props, output_folder_attribute.value), renderset_context
        )

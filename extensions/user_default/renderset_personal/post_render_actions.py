...
class PostRenderAction(bpy.types.PropertyGroup):
    ...
    render_layer_type: bpy.props.EnumProperty(
        ...
        items=[
            ...
            None,
            (
                compositor_helpers.RenderPassType.MULTILAYER_EXR.value,
                "Custom EXR Multilayer output",
                "Used for output from custom EXR Multilayer output nodes",
            ),
        ],
    ...
    def execute_action(
        ...
        render_pass_filename_map: typing.Dict[str, typing.Set[str]],
        ...
        if self.action_type in {
            ...
            no_prefix_source_filename = renderset_context.generate_output_filename(
                is_movie_format,
                time=time,
            ...
            found_outputs = render_pass_filename_map.get(render_pass, None)
            if found_outputs is None:
                logger.warning(
                    f"PostRenderAction of type {self.action_type} cannot be executed, "
                    f"no output of render pass '{render_pass}' was not found."
                )
                return

            for output in found_outputs:
                source_filename, ext = os.path.splitext(output)
                assert source_filename.endswith(no_prefix_source_filename)
                # Check for possible custom output node prefix
                prefix_split = source_filename.removesuffix(no_prefix_source_filename).rsplit(
                    "_", 1
                )

                prefix = prefix_split[0] if len(prefix_split) == 2 else None
                ...
                source_file = os.path.join(final_output_folder_path, f"{source_filename}{ext}")
                ...
                if self.action_type == PostRenderActionType.COPY_OUTPUT_FILE.value:
                    ...
                    polib.utils_bpy.move_file(source_file, target_file)
            ...
            # Simple try to ensure the webhook URL is valid.
            webhook_path = self.webhook_url.replace("https://hooks.slack.com", "")
            ...
            try:
                connection = http.client.HTTPSConnection("hooks.slack.com", timeout=5)
                ...
                connection.request(
                    'POST', webhook_path, body=json.dumps(body_json), headers=headers
                )
                ...
                if response.status < 300:
                    ...
                    logger.error(
                        f"PostRenderAction of type {self.action_type} failed, "
                        f"webhook_url: {webhook_path}, status: {response.status}, reason: {response.reason}"
                    )
                connection.close()
            except http.client.HTTPException as e:
                logger.exception(f"Connecting to slack webhook failed:")
            ...
            if not os.path.isfile(self.sound_filepath):
                ...
                try:
                    ...
                    logger.warning(
                        "Wanted to play a sound as part of a post render action but "
                        "the 'aud' module is not available. Skipping."
                    )
            ...
            raise ValueError(f"Unknown post render action type: {self.action_type}")

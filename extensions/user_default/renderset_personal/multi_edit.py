    ...
    def draw(self, context: bpy.types.Context) -> None:
        ...
        for i, rset_context in enumerate(context.scene.renderset_contexts):
            if i == context.scene.renderset_context_index:
                ...
                renderset_context.draw_color_and_name(context, name_col, rset_context)
            else:
    ...
    def draw(self, context: bpy.types.Context) -> None:
        ...
        for hunk_key, _ in flattened_dict_patch.items():
            ...
            prop_path, initial_prop_container, initial_prop_path = (
                sync_overrides.infer_initial_prop_container_and_path(
                    hunk_key, renderset_context_utils.get_active_renderset_context(context)
                )
            )
        ...
        try:
            ...
            if len(flattened_dict_patch) == 0:
                ...
                for rset_context in context.scene.renderset_contexts:
                    ...
                    # We need to apply the rset_context's properties to itself directly
                    # because we can access some of them from the UI even if the context is not active
                    # For example the icon in the UI list changes based on the render type
                    # and we can open each context's results folder
                    rset_context.apply_self_props()
